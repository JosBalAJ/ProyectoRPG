...
Character* Combat::getTarget(Character* attacker) {
    ...
    if (!foundTarget){
        cout << "No valid target found. Attacker: " << attacker->getName() << endl;
    }
    return target;
}

void Combat::doCombat() {
    cout<< "\n" << "************** START OF THE BATTLE *************" << endl;
    combatPrep();
    int round = 1;
...
void Combat::registerActions(vector<Character*>::iterator participantIterator) {
    ...
    while(participantIterator != participants.end()) {
        if((*participantIterator)->getIsPlayer()) {
            ...
            if (playerAction.action != nullptr && !((Player*) *participantIterator)->isDefending){
                actionQueue.push(playerAction);
            }else{
                playerAction.action();
                ((Player*) *participantIterator)->isDefending = false;
                break;
            }
        } else {
